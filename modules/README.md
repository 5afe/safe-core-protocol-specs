...
```javascript
    const ACCOUNT_MSG_TYPEHASH = hre.ethers.keccak256(toUtf8Bytes("AccountMessage(bytes message)"));
    const domainSeparator = await account.domainSeparator();
    const dataHash = hre.ethers.keccak256(toUtf8Bytes("data-to-be-signed"));
    const messageHash = hre.ethers.keccak256(
        hre.ethers.solidityPacked(["bytes1", "bytes1", "bytes32", "bytes32", "bytes32"], ["0x19", "0x01", domainSeparator, ACCOUNT_MSG_TYPEHASH, dataHash]),
    );

    const signatures = generateSignaturesUsingWallet(messageHash);
    const result = await account.isValidSignature.staticCall(dataHash, signatures);
```
...
A malicious actor can change the routing of signature validation flow by adding/removing the 4 bytes of signature selector. Under such cases the signature validation should fail and revert the transaction.

Suppose, a malicious removes the 4 bytes of signature selector from a payload meant for domain specific validation flow, submits the modified payload for validation such that validator manager uses default signature validation flow. In such case, as the account expects `messageHash` to be generated by including account's domain separator, the validation would revert as the hash passed to the account for validation would be different from the hash used for signing the message.

Similarly, if a malicious actor adds 4 bytes of signature selector to a payload meant for default signature validation flow, submits the modified payload for validation such that validator manager uses domain specific signature validation flow. Even if permitted by the registry and account has set a signature validator that matches account's domain separator, the validation would revert when validator manager would check for the hash.
